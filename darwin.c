...
int toggleProxy(bool turnOn, const char* proxyHost, const char* proxyPort)
{
  ...
  NSString* nsProxyPort = [[NSString alloc] initWithCString: proxyPort encoding:NSUTF8StringEncoding];
  NSString* nsOldProxyHost;
  NSString* nsOldProxyPort;
  ...
  for (long i = 0; i < CFArrayGetCount(networkServicesArrayRef); i++) {
    ...
    if(turnOn == true) {
      ...
      if ([nsProxyHost isEqualToString:nsOldProxyHost] && [nsProxyPort isEqualToString:nsOldProxyPort]) {
